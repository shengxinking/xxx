...
#define THREAD_CLOCK 6

#define THREAD_MAX	6	/* the max type value */
/**
...
//	objpool_t	*ssnpool;	/* connection_t pool */
	connection_table_t	*sesstbl;	/* connection table for each thread */
...
typedef struct _recv {
	connection_t	**inq;		/* input queue */
	connection_t	**inq2;		/* secondary input queue */
	pthread_mutex_t	lock;		/* lock for input queue */
...
/**
 *	Add a new connection into work thread.
 *
 *
 *	Return 0 if success, -1 on error.
 */
